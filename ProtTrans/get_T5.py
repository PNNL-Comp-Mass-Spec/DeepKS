...
class GetT5:
    ...
    class T5Instance:
        ...
        def get_embeddings(self, seqs: list[str] | numpyArray[np.str_]) -> list[list[float]]:
            ...
            logger.status(f"Getting embeddings from the T5 model from scratch for {len(seqs)} sequences.")
        ...
        def _get_embeddings_core(
            ...
            with torch.no_grad():
                ...
                for i in CustomTqdm(range(0, len(seqs), chunk_size), desc="T5 Embedding Progress", position=1):
                    ...
                    res += embedded
            self.new_embs = dict(zip(seqs, res))
            return res


def test():
    sample_seqs = ["ACDEF", "PEPTIDE", "SEQWENCE", "PROTEIN", "PROTEEN"]
    t5_embedder = GetT5()
    logger.debug(emb := t5_embedder.get_t5(sample_seqs, device=torch.device("cuda:4")))
