...
def generate_real_helper(section_df: pd.DataFrame, input_fn: str, kin_seq_file: str, percentile: float, write_file=True):
    ...
    for kin in start_dict:
        extra = 0 if ((end_dict[kin] - start_dict[kin] + 1) % 2 == 0) else random.choice([-0.5, 0.5]) # random.choice([0, 1])
        k = (((end_dict[kin] - start_dict[kin]) + 1)/2) + extra
        assert(k == int(k))
        k = int(k)
        rs = random.sample(range(start_dict[kin], end_dict[kin] + 1), k = int(k))
        for x in rs:
            if x in to_pop_inds:
    ...
    target = all_data.reset_index().reset_index(drop=True)
    ...
    derangement = [x if x is not None else len(decoy) for x in get_groups_derangement4(order, sizes, kin_seq_file, percentile)]
...
if __name__ == "__main__":
    if mode == "no_alin":
        kin_seq_file = "../raw_data/kinase_seq_792.txt"
    else:
        kin_seq_file = "../raw_data/kinase_seq_alin_792.txt"

    generate_real("../raw_data_22068.csv", kin_seq_file, {
