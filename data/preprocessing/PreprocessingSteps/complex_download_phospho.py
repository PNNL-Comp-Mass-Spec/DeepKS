...
async def complex_download_phospho(): # pragma: no cover
    ...
    available_organisms_to_url = {
        re.findall(r"<em>(.+)<\/em>", re.sub(r"\s+", "", x))[
            0
        ]: f"http://kinase.com/web/current/kinbase/genes/speciesID/{re.findall('/SpeciesID/([0-9]+)/', x)[0]}/"
        for x in available_organisms_raw
    }
