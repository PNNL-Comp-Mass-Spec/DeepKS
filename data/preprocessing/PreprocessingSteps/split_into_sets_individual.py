...
def split_into_sets(kin_fam_grp_file, raw_input_file, tgt = 0.3, get_restart = True, restart_idx = None, num_restarts = 25):
    ...
    def get_kin_assignments_from_state(states: np.array):
    ...
    def core_ai(tgt, random_state, group, ret_score = True):
        ...
        if ret_score == True:
            fit = mlr.CustomFitness(hill_climbing_score, problem_type = 'discrete', targets = [1-tgt, tgt/2, tgt/2], group = group)
        else:
            fit = mlr.CustomFitness(hill_climbing_score, problem_type = 'discrete', targets = [1-tgt, tgt/2, tgt/2], group = group)
        opt = mlr.DiscreteOpt(num_fams, fit, maximize = False, max_val = 3)
        res = mlr.simulated_annealing(opt, max_attempts=1000, max_iters=1000, random_state = random_state, init_state=[0 for _ in range(num_fams)])
        return res

    def algorithm4(tgt, group, verbose = True, num_restarts = 100):
        scores = []
        states = []
        ...
        state = states[mai]
        if verbose:
            tqdm.write("Best score for group {} was {} achieved by random state {} and the restart scores had an SD of {:3.2f}".format(group, ma, mai, sd))
    ...
    def algorithm5(tgt, verbose = True, num_restarts = 100):
        ...
        group_sd = []
        group_states = {}
        for i, group in enumerate(tq := tqdm(grp_to_num_sites.keys(), position=0, leave=True)):
        ...
        # hill_climbing_score(group_states['STE'], group = 'STE', targets = [0.7, 0.15, 0.15], unsquared = [])
        print(f"Overall Score = {sum(group_scores)}")
        return group_states

    if get_restart:
        states = algorithm5(tgt, num_restarts=num_restarts, verbose=True)
        tk, vk, tek = get_kin_assignments_from_state(states)
        vk = set(vk)
        tek = set(tek)
        ...
        print(f"Unsquared Score = {true_acc} ({true_acc/sum(grp_to_num_sites.values())*100}%)")
        vk = list(vk)
        tek = list(tek)
