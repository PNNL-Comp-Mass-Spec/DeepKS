...
def main():
    ...
    upload_field_default = driver.find_element(By.XPATH, "/html/body/app-root/app-content-wrapper/main/app-score-site/div[2]/div[1]/div/app-file-upload/label/input")
    ...
    wait.until(EC.presence_of_element_located((By.XPATH, "/html/body/app-root/app-content-wrapper/main/app-score-site/div[2]/div[1]/div/app-file-upload/p")))

    print("Upload complete; about to click submit...")

    # Find the submit button on the form and click it
    submit_button = driver.find_element(By.XPATH, "/html/body/app-root/app-content-wrapper/main/app-score-site/div[2]/div[2]/div[2]/button")
    submit_button.click()
    print("Submit button clicked...")

    # Wait for the results to load
    wait = WebDriverWait(driver, WAIT_TIMEOUT)
    wait.until(EC.presence_of_element_located((By.XPATH, "/html/body/app-root/app-content-wrapper/main/app-score-site/app-sites-result/div/div/div[1]/div/div[1]/label")))

    print("Results visible...")
    # For each page of results...
    try:
        for p in range(1, max_page := 1 + max([int(x.get_attribute("innerHTML")) for x in driver.find_element(By.XPATH, "/html/body/app-root/app-content-wrapper/main/app-score-site/app-sites-result/div/div/div[1]/div/div[2]/span").find_elements(By.XPATH, ".//*") if x.get_attribute("innerHTML").isnumeric()])):
            # For each table of results, scrape the table
            driver.execute_script("window.scrollTo(0, 0)")
            for i in range(1, max_tab := 1 + len(driver.find_element(By.XPATH, "/html/body/app-root/app-content-wrapper/main/app-score-site/app-sites-result/div/div/div[1]/div/table/tbody").find_elements(By.TAG_NAME, "tr"))):
                print(f"Starting download (Page {p}/{max_page - 1}, Table {i}/{max_tab - 1})...")
                driver.execute_script("window.scrollTo(0, 0)")
                # Find the next button and click it
                tab_bttn = driver.find_element(By.XPATH, f"/html/body/app-root/app-content-wrapper/main/app-score-site/app-sites-result/div/div/div[1]/div/table/tbody/tr[{i}]")
                which_short_site = "".join([driver.find_element(By.XPATH, f"/html/body/app-root/app-content-wrapper/main/app-score-site/app-sites-result/div/div/div[1]/div/table/tbody/tr[{i}]/td/span[{s}]").get_attribute("innerHTML") for s in [1, 2, 3]])
                tab_bttn.click()
