...
model = KinaseSubstrateRelationshipNN(
    ...
    site_param_dict=site_param_dict,
    kin_param_dict=kin_param_dict,
).to(device)
...
rax.scatter(
    fpr[[round(x, 3) for x in thresholds.tolist()].index(0.501)],
    0.739,
    label="Model's Validation Accuracy (50% Prob. Thresh.)",
    color="violet",
    s=10,
)
