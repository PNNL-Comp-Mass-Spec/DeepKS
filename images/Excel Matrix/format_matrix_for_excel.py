...
def format_for_excel(mtx_file, raw_data_file):
    mtx = pd.read_csv(mtx_file, index_col=0).applymap(lambda x: np.round(x*100, 2) if isinstance(x, Union[int,float]) else x)
    ...
    mtx.rename(columns={"Unnamed: 0": "uniprot_id"} | {current: new for current, new in zip(mtx.columns, uniprots)}, inplace=True)
    ...
    row_concat_df = pd.DataFrame({"gene_name": 3*[pd.NA] + gene_names, "organism": 3*[pd.NA] + organisms, "uniprot_id": 3*[pd.NA] + uniprots}).T
