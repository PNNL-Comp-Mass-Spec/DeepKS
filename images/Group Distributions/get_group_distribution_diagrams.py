...
def get_overall_plot(counts, savefig=False):
    ...
    baxes.bar(
        labs,
        align="center",
        height=[cnt_vl[la] for la in labs],
        color="aqua",
        alpha=1,
        bottom=[cnt_tr[la] for la in labs],
        label="Dev",
    )
    baxes.bar(
        labs,
        align="center",
        height=[cnt_te[la] for la in labs],
        color="blue",
        alpha=1,
        bottom=[cnt_tr[la] + cnt_vl[la] for la in labs],
        label="Test",
    )
...
def get_proportional_plot(counts, savefig=False):
    ...
    raw_data = {
        "greenBars": [cnt_tr[la] for la in labs],
        "orangeBars": [cnt_vl[la] for la in labs],
        "blueBars": [cnt_te[la] for la in labs],
    }
    ...
    plt.hlines(
        70, -100, 100, ["black"] * 2, "dashed", linewidth=1, label=textwrap.fill("Train set proportion goal", 15)
    )
...
if __name__ == "__main__":
    ...
    os.chdir(where_am_i)
    print(where_am_i)
    data = get_group_distribution_diagrams(
        split_into_sets_individual_deterministic_top_k.get_assignment_info_dict(
            "../../data/preprocessing/kin_to_fam_to_grp_817.csv",
            "../../data/raw_data/raw_data_22473.csv",
            *[f"../../data/preprocessing/{x}_kins.json" for x in ["tr", "vl", "te"]],
        )
    )
