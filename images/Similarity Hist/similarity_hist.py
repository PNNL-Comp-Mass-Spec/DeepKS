...
def main(kin, org):
    ...
    def execute(token):
        ...
        # Just for Bins
        hist_params = dict(bins=bins, alpha = 1, color = "#58b368", label = "Similarity Distribution", edgecolor='#248f69', align = 'mid')
        ...
        plt.title(f"Distribution of Pairwise Kinase Similarities\nBetween {token.split('|')[0]} and {len(data)} Kinases (All Species)")
        plt.ylabel("Number of Kinases")
        plt.xlabel("Percent Identity (Clustal Omega)")
        plt.xticks(*[xt := list(range(0, 101, 5))]*2)
        plt.yticks(*[yt := list(range(0, int(np.ceil(1.1*np.max(factor_hist_a[0]))), max(10, 10**int(np.log10(1.1*np.max(factor_hist_a[0])/25)))))]*2)
        plt.xlim(min(xt), max(xt))
        plt.ylim(min(yt), max(yt))
        plt.vlines(pct, 0, max(yt), color = "teal", linestyle = 'dashed', label = f"95% Similarity Threshold\n(@ {pct:.3}% Identity)")
...
def add_gmm_curves(n, data, hist = None, replot_hist = False, param_dict = None, lines_params = {'color': 'lime'}, verbose = True, init_means = None):
    if len(data.shape) < 2:
