...
class KinaseSubstrateRelationshipATTNWSELF(KSR):
    def __init__(
        ...
        self.self_attn_site = MHAttention(embed_dim=self.site_param_dict["out_channels"][-1], num_heads=attn_num_heads)
        self.self_attn_kin = MHAttention(embed_dim=self.kin_param_dict["out_channels"][-1], num_heads=attn_num_heads)
    ...
    def forward(self, site_seq, kin_seq):
        ...
        site_self_attn_out, _ = self.self_attn_site(transp_out_site, transp_out_site)
        kin_self_attn_out, _ = self.self_attn_kin(transp_out_kin, transp_out_kin)

        attn_out, _ = self.attn(site_self_attn_out, kin_self_attn_out)
