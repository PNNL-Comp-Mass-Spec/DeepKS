...
def exhaustive_pairwise_knn(known_true_classes: np.ndarray, queries: np.ndarray, k: int = 5, tie_policy: typing.Literal["random", "alpha", "raise"] = "random", best_score_is_higher: bool = True):
...
if __name__ == "__main__":
    with open("../data/preprocessing/tr_kins.json") as trf, open("../data/preprocessing/vl_kins.json") as vlf, open("../data/preprocessing/te_kins.json") as tef:
        pseudo_training_symbols = [x.replace("(", "").replace(")", "").replace("*", "") for x in json.load(trf) + json.load(vlf)]
    ...
    # print(queries)
    predictions = exhaustive_pairwise_knn(np.array(class_df['Group'].tolist()), queries_test_block.values, k=15, tie_policy='random')
