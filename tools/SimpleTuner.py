...
class SimpleTuner:
    def __init__(self, num_sim_procs, train_fn, config_dict, num_samples, random_seed=0, collapse=[[]], num_gpu=None, max_output_width = 512):
        ...
        assert all(isinstance(x, list) for x in config_dict.values()), "Not all hyperparameters are in a list. (E.g., `'num_epochs':1` instead of `'num_epochs':[1]`)"

        cart_prod = list(itertools.product(*list(config_dict.values())))
        self.num_samples = min(num_samples, len(cart_prod))
        try: 
            assert(self.num_samples == num_samples)
        except AssertionError:
            warnings.warn('num_samples exceeds the number of possible combinations. Using all possible combinations (%d) instead.' % len(cart_prod))
    ...
    def go(self, *args):
        ...
        with multiprocessing.Pool(self.num_sim_procs, initializer=self.init_pool) as pool:
            try:
                results = pool.starmap(self.display_intermediates, [[cd, dispatch_num, *args] for cd, dispatch_num in zip(self.sampled_config_dicts, range(len(self.sampled_config_dicts)))])
    ...
    def display_final_results(self, results):
        cols = ['dispatch_num'] + list(self.sampled_config_dicts[0].keys()) + ['acc_vals', 'acc', 'loss', 'acc_std', 'loss_std']
        ...
        pt.max_table_width = self.max_output_width
        rows = [[i] + list(self.sampled_config_dicts[i].values()) + [results[i][0]] + [sigfig.round(x, sigfigs=3) for x in list(results[i])[1:]] for i in range(len(self.sampled_config_dicts))]
    ...
    def table_intermediates(config, *args):
        ...
        pt.add_row(list(config.values()) + [acc_vals] + [sigfig.round(x, sigfigs = 3) for x in [acc, loss, acc_std, loss_std]])
        print("", flush=True)
        print(pt, flush=True)
        print("", flush=True)
    ...
    def display_intermediates(self, conf, dispatch_num, *args):
        try:
            ...
            for i in range(len(acc_vals)):
                acc_vals[i] = sigfig.round(acc_vals[i], sigfigs=3)
            pt.add_row([dispatch_num] + list(conf.values()) + [acc_vals] + [sigfig.round(x, sigfigs = 3) for x in [acc, loss, acc_std, loss_std]])
            print("", flush=True)
            print(pt, flush=True)
            print("", flush=True)
...
def mainfn(cd):
        strr = f"My batch size is {cd['batch_size']}; "\
