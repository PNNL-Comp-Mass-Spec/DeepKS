...
def nice_printer(
    obj, max_depth=5, max_items=10, min_singleton_width = 20, max_width=int(os.get_terminal_size().columns*0.8), initial_indent=4, subsequent_indent=4, file= sys.stdout
):
...
def _base_iterable_shortener(node, max_items, max_str_len, cur_indent):
    ...
    for e, v in enumerate(node):
        if isinstance(v, dict):
            _base_dict_shortener(v, max_items, max_str_len, cur_indent + SUBSEQUENT_INDENT, addl_end = "\n" + " " * (cur_indent))
            ...
            _base_singleton_shortener(v, max_str_len, ", " if e != len(node) - 1 else "", next_len = (next_len:=len(str(node[e+1])) if e < len(node) - 1 else len(str(node[e]))), cur_indent=cur_indent, indent_exception=e==0)
...
def _base_singleton_shortener(node, max_str_len, sep, indent="", next_len = 0., cur_indent=0, indent_exception=False):
