...
def parsing() -> dict[str, Union[str, None]]:
    ...
    parser.add_argument(
        "--train", type=str, help="Specify train file name", required=False, metavar="<train_file_name.csv>"
    )
    parser.add_argument(
        "--val", type=str, help="Specify validation file name", required=False, metavar="<val_file_name.csv>"
    )
    parser.add_argument(
        "--test", type=str, help="Specify test file name", required=False, metavar="<test_file_name.csv>"
    )
    parser.add_argument(
        "--load", type=str, help="Specify path from which to load", required=False, metavar="<load/file/name>"
    )
    parser.add_argument(
        "--load-include-eval",
        type=str,
        help="Specify path from which to load",
        required=False,
        metavar="<load/file/name>",
    )
    ...
    if args["load_include_eval"] is None:
        ...
        if args["load"] is not None:
            ...
            assert all(
                [args[x] is not None for x in ["train", "val"]]
            ), "If not specifying --load argument, must` specify --train and --val arguments."
            for f in [train_filename, val_filename]:
                try:
                    # assert 'formatted' in f, "'formatted' is not in the train filename. Did you select the correct file?"# TODO
                    pass
        ...
        assert all(
            [args[x] is None for x in ["train", "val", "test", "load"]]
        ), "If specifying --load-include_eval argument, cannot specify --train, --val, --test, nor --load arguments."
    return args
