...
def gather_data(
    ...
    if ret_info:
        return max_kin_length, tok_dict
    ...
    if trf > 0:
        train_loader = torch.utils.data.DataLoader(
            model_utils.ToyDataset(X_train, X_train_kin, y_train),
    ...
    if vf > 0:
        val_loader = torch.utils.data.DataLoader(
            model_utils.ToyDataset(X_val, X_val_kin, y_val), batch_size=len(X_val)
        )
    else:
        val_loader = None
    if tuf > 0:
        tune_loader = torch.utils.data.DataLoader(
            model_utils.ToyDataset(X_tune, X_tune_kin, y_tune), batch_size=len(X_tune)
        )
    else:
        tune_loader = None
    if tef > 0:
        test_loader = torch.utils.data.DataLoader(
            model_utils.ToyDataset(X_test, X_test_kin, y_test), batch_size=len(X_test)
        )
    else:
        test_loader = None
...
def get_info(target_file, decoy_file="", n_gram=1):
    max_length0, tok_dict0 = gather_data(target_file, ret_info=True, n_gram=n_gram)
    if decoy_file != "":
