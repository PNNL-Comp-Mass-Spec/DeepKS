...
def gather_data(
    ...
    y_train, y_val, y_tune, y_test = tuple(
        [torch.IntTensor(d_set[class_col].values).to(device) for d_set in [data_train, data_val, data_tune, data_test]]
    )

    if trf > 0:
        train_loader = torch.utils.data.DataLoader(
            model_utils.KGroupDataset(X_train_kin, y_train),
    ...
    if vf > 0:
        val_loader = torch.utils.data.DataLoader(
            model_utils.KGroupDataset(X_val_kin, y_val), batch_size=len(X_val_kin) if X_val_kin is not None else 0
        )
    else:
        val_loader = None
    if tuf > 0:
        tune_loader = torch.utils.data.DataLoader(
            model_utils.KGroupDataset(X_tune_kin, y_tune), batch_size=len(X_tune_kin) if X_tune_kin is not None else 0
        )
    else:
        tune_loader = None
    if tef > 0:
        test_loader = torch.utils.data.DataLoader(
            model_utils.KGroupDataset(X_test_kin, y_test), batch_size=len(X_test_kin) if X_test_kin is not None else 0
        )
    else:
        test_loader = None
    ...
    return (train_loader, val_loader, tune_loader, test_loader), {
        "kin_orders": {
            "train": data.loc[train_ids]["Symbol"].to_list(),
            "val": data.loc[val_ids]["Symbol"].to_list(),
            "tune": data.loc[tune_ids]["Symbol"].to_list(),
            "test": data.loc[test_ids]["Symbol"].to_list(),
        },
        "classes": classes,
...
if __name__ == "__main__":
    n = 3
    max_kinase_length, tok_dict = get_info(
        "data/raw_data_train_8151_formatted.csv", "data/raw_data_held_out_998_formatted.csv", n
    )
    (a, b, c, d), _ = gather_data(
        "data/raw_data_train_8151_formatted.csv", maxsize=max_kinase_length, tokdict=tok_dict, n_gram=n
    )
    print()
